...
import { WeightBracket, VehicleType, WEIGHT_BRACKET_RANGES } from '../types/logistics.types';
...
export class MerchantLogisticsService {
   ...
   */
  public static getWeightBracketFromWeight(weightKg: number): WeightBracket {
    for (const [bracket, range] of Object.entries(WEIGHT_BRACKET_RANGES)) {
   ...
   */
  public static async getMerchantFBURequests(merchantId: string) {
    try {
      // ✅ Use correct case: FBUWarehouseRequest (capital F)
      ...
      console.error('Error fetching FBU requests:', error);
      return [];
    }
  }
   ...
   */
  public static async getMerchantFBURequest(merchantId: string, requestId: string) {
    const request = await prisma.FBUWarehouseRequest.findFirst({
      where: { id: requestId, merchantId }
    });
    
    if (!request) {
      throw new Error('FBU request not found');
    }
  ...
  /**
   * Confirm pickup scheduled (merchant acknowledges)
   */
  public static async merchantConfirmPickupSchedule(
    ...
    if (request.status !== 'pending_pickup') {
      throw new Error(`Cannot schedule pickup for request with status: ${request.status}`);
    }
