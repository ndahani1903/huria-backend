  ...
  async (req: any, res: any) => {
    try {
      ...
      const data = {
        ...
        businessLat: req.body.businessLat
          ? parseFloat(req.body.businessLat)
          : undefined,
        businessLng: req.body.businessLng
          ? parseFloat(req.body.businessLng)
          : undefined,
        nidaCardImage: (files.nidaCardImage as any)?.[0]?.path,
        passportPhoto: (files.passportPhoto as any)?.[0]?.path,
        selfieImage: (files.selfieImage as any)?.[0]?.path,
        businessLicenseImage:
          (files.businessLicenseImage as any)?.[0]?.path,
...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      const result = await KYCService.freezeUser(
        userId,
        userType,
        req.user.id,
        reason || 'Admin action'
...
// Unfreeze user
router.post(
  '/admin/unfreeze/:userType/:userId',
  authMiddleware,
  requireRole('admin'),
  async (req: any, res: any) => {
    try {
      ...
      const result = await KYCService.unfreezeUser(
        userId,
        userType,
        req.user.id,
        reason
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      ...
      const logs = await KYCService.getKYCAuditLogs(
        userId as string,
