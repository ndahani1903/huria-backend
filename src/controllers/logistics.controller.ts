...
export class LogisticsController {
  ...
  static async evaluateCart(req: AuthRequest, res: Response) {
    try {
      ...
      // Get product logistics info for all items in cart
      const productIds = cartItems.map((item: any) => item.productId);
      const productsInfo =
        await LogisticsService.getBatchLogisticsInfo(productIds);
      
      // Check if all products are from same merchant
      const uniqueMerchants = [
        ...new Set(productsInfo.map(p => p.merchantId))
      ];
      ...
      if (!merchantInfo) {
        return res.status(400).json({
          error: 'Unable to determine merchant information'
        });
      }

      if (!merchantInfo.merchantLocation) {
        return res.status(400).json({
          error: 'Merchant pickup location not configured'
        });
      }

      // Get merchant details separately.
      // This avoids relying on a merchant relation that is not
      // part of the LogisticsService return type.
      const merchant = await prisma.merchant.findUnique({
        where: { id: merchantInfo.merchantId },
        select: {
          businessName: true,
          name: true
        }
      ...
      const cartItemsWithLogistics: CartItemLogistics[] =
        cartItems.map((item: any) => {
          ...
          return {
            ...
            weightBracket:
              productInfo?.weightBracket || 'LIGHT',
            allowedVehicles:
              productInfo?.allowedVehicles ||
              ['motorcycle', 'bajaj', 'truck']
          };
        });
   ...
   */
  static async updateProductLogistics(
    req: AuthRequest,
    res: Response
  ) {
    try {
  ...
  : req.params.productId;
      ...
      console.log(
        '📦 Updating product logistics:',
        { productId, weightBracket, allowedVehicles }
      );

      console.log('👤 User from token:', req.user);

      // Get merchant by userId (not merchantId from token)
      const merchant = await prisma.merchant.findUnique({
      ...
      const updated =
        await MerchantLogisticsService.updateProductLogistics(
          productId,
          merchant.id,
          {
            weightBracket,
      ...
      console.error(
        'Update product logistics error:',
        error
      );
   ...
   */
  static async getMerchantFBURequests(
    req: AuthRequest,
    res: Response
  ) {
    try {
      ...
      // Call the service correctly
      // Pass merchant.id, not merchant object
      const requests =
        await MerchantLogisticsService.getMerchantFBURequests(
          merchant.id
        );
      ...
      console.error(
        'Get merchant FBU requests error:',
        error
      );
   ...
   */
  static async confirmPickupSchedule(
    req: AuthRequest,
    res: Response
  ) {
    try {
  ...
  : req.params.requestId;
      ...
      const updated =
        await MerchantLogisticsService.merchantConfirmPickupSchedule(
          merchant.id,
          requestId,
  ...
  static async getWeightBracket(
    ...
    try {
      ...
      if (!weightKg) {
        return res.status(400).json({
          error: 'weightKg query param required'
        });
      }
      
      const bracket =
        MerchantLogisticsService.getWeightBracketFromWeight(
          Number(weightKg)
        );
