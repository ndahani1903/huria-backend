...
export class MerchantTypeService {
  ...
  static async getNearbyMerchants(
    ...
    console.log(
      `🔍 getNearbyMerchants called with:`,
      {
        userLat,
        userLng,
        type,
    ...
    for (const merchant of merchants) {
      ...
      const reviewCount =
        await prisma.storeReview.count({
          where: {
  ...
  static async getMerchantDetails(
    ...
    const merchant =
      await prisma.merchant.findUnique({
        ...
        include: {
          ...
          // IMPORTANT:
          // operatingHours is a Prisma relation in the
    ...
    const totalReviews =
      ratingAggregate._count;

    // ========================================================
    // DISTANCE / ORDERABILITY
    // ========================================================
    ...
    if (
      ...
      merchant.pickupLat !== null &&
      merchant.pickupLng !== null
    ) {
      distanceKm =
        calculateHaversineDistance(
          {
    ...
    const merchant =
      await prisma.merchant.update({
        ...
        data: {
          ...
          isAcceptingOrders:
            updates.isAcceptingOrders,

          maxDeliveryRadiusKm:
            updates.maxDeliveryRadiusKm,

          hasBarcodeScanner:
            updates.hasBarcodeScanner,

          aisleCategories:
            updates.aisleCategories,
  ...
  static async updateStock(
    ...
    // If stock is low, a notification/socket event
    // could be triggered here.
    if (
      newStock < 10 &&
      newStock !== oldStock
    ) {
      // Future low-stock notification
    }
